...
// Legacy function for backward compatibility with old API format
export function displaySuggestionsLegacy(data: {
  skills: Array<{
    id: number;
    name: string;
    title: string;
  ...
  const converted: SuggestionResult = {
    ...
    total: data.total,
    matchedPackages: data.matched_packages,
    matchedInstalledSkills: data.matched_installed_skills,
    sourcedFrom: "own-api",
  };

  displaySuggestions(converted);
}
