...
async function processSuggestRequest(
  packageList: string[],
  ...
  const matchedInstalledSkills = matchedInstalledRows.map((row) => row.name);
  ...
  if (packageList.length > 0) {
    ...
    if (matchedInstalledSkillIds.length > 0) {
      packageConditions.push(notInArray(skills.id, matchedInstalledSkillIds));
    }
  ...
  const installedPackagesRows =
    matchedInstalledSkillIds.length > 0
      ? await db
          ...
          .where(inArray(packageSkillAsscn.skillId, matchedInstalledSkillIds))
      : [];
...
export const skillsRoutes = new Elysia().group("/api/v1/skills", (app) =>
  app
    .post("/suggest", async ({ body, set }) => {
      const request = body as SuggestRequest;
      const packageList = normalizeArray(
        Array.isArray(request.packages) ? request.packages : [],
      );
      const installedSkills = normalizeArray(
        Array.isArray(request.installed_skills)
          ? request.installed_skills
          : [],
      );
      const limitValue = Number(request.limit ?? 10);
      const limit =
      ...
      return await processSuggestRequest(packageList, installedSkills, limit);
    })
